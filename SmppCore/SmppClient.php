...
class SmppClient
{
    ...
    public static $csmsMethod = SmppClient::CSMS_16BIT_TAGS;
     ...
     * 
     * @param SmppAddress $from                 From
     * @param SmppAddress $to                   To
     ...
     */
    public function sendSMS(SmppAddress $from,
                            SmppAddress $to,
                            $message,
                            $tags=null,
                            ...
                            $validityPeriod=null)
    {
        ...
        if ($doCsms) {
            if (self::$csmsMethod == SmppClient::CSMS_PAYLOAD) {
                $payload = new SmppTag(SmppTag::MESSAGE_PAYLOAD, $message, $msgLength);
                ...
                if (self::$csmsMethod == SmppClient::CSMS_8BIT_UDH) {
                    $seqnum = 1;
                    ...
                    $sarMsgRefNum = new SmppTag(SmppTag::SAR_MSG_REF_NUM, $csmsReference, 2, 'n');
                    $sarTotalSegments = new SmppTag(SmppTag::SAR_TOTAL_SEGMENTS, count($parts), 1, 'c');
                    $seqnum = 1;
                    foreach ($parts as $part) {
                        $sartags = array($sarMsgRefNum, $sarTotalSegments, new SmppTag(SmppTag::SAR_SEGMENT_SEQNUM, $seqnum, 1, 'c'));
                        $res = $this->submit_sm(
                            $from,
                            $to,
                            $part,
     ...
     * @return string message id
     */
    protected function submit_sm(SmppAddress $source,
    ...
    {
        $limit = (SmppClient::$csmsMethod == SmppClient::CSMS_8BIT_UDH) ? 255 : 65535;
    ...
    protected function parseSMS(SmppPdu $pdu)
    {
        ...
        $source = new SmppAddress($sourceAddress, $sourceAddressTon, $sourceAddressNpi);

        $destinationAddressTon = next($ar);
        $destinationAddressNpi = next($ar);
        $destinationAddress = $this->getString($ar, 21);
        $destination = new SmppAddress($destinationAddress, $destinationAddressTon, $destinationAddressNpi);
        ...
        if (($esmClass & SMPP::ESM_DELIVER_SMSC_RECEIPT) != 0) {
            $sms = new SmppDeliveryReceipt($pdu->id, $pdu->status, $pdu->sequence, $pdu->body, $serviceType, $source, $destination, $esmClass, $protocolId, $priorityFlag, $registeredDelivery, $dataCoding, $message, $tags);
            $sms->parseDeliveryReceipt();
        } else {
            $sms = new SmppSms($pdu->id, $pdu->status, $pdu->sequence, $pdu->body, $serviceType, $source, $destination, $esmClass, $protocolId, $priorityFlag, $registeredDelivery, $dataCoding, $message, $tags);
        }
    ...
    protected function readPDU()
    {
        ...
        return new SmppPdu($headers['commandId'], $headers['commandStatus'], $headers['sequenceNumber'], $body);
    }
    ...
    protected function parseTag(&$ar)
    {
        ...
        $tag = new SmppTag($unpackedData['id'], $value, $unpackedData['length']);
        if ($this->debug) {
